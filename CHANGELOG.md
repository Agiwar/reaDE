...
- `sql`: the `bind` and `ident` template filters and the `RenderedQuery`
  model — `bind` registers values as bound parameters and emits the
  dialect's PEP 249 placeholder (`:name` for SQLite, `%(name)s` for
  PostgreSQL/MySQL); `ident` allowlists and dialect-quotes identifiers.
- `data_io`: `read_csv` / `write_csv` stream dict rows keyed by header —
  values stay raw strings, ragged rows raise, CSV parse/shape failures
  map to `DataIoError`, `OSError` passes through.
- Machine-checked development gates: public-API snapshot test,
  import-linter layered-dependency contract, 90% coverage floor.

### Changed

- Breaking: `render_template` is rebuilt around the render contract —
  `render_template(template_name, dialect, context=None, /, *,
  search_paths=None) -> RenderedQuery` replaces the Phase-0 signature;
  templates render with `StrictUndefined` and load only from the
  packaged directory plus caller-supplied `search_paths`; missing
  search-path entries fail loud.
- Breaking (pre-registered bundle; design-review note in PR #31):
  ...
  `ConnectionInterface` as a read-only property; `execute_query` becomes
  `execute_query(connector, sql, params=None)`, carrying bound
  parameters through the SDK, and — like `RowCountRule.evaluate` and
  `VolumeDimension.assess` — now accepts any `ConnectionInterface`
  implementation (was `ConnectionBase[Any]`); `merge_env_overrides`'s
  `environ` becomes keyword-only with a call-time default.
